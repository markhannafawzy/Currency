...
/// Represents a response to a `NetworkLayerManager.request`.
...
public extension Response {
    ...
    /**
     ...
     - throws: `NetworkLayerError.statusCode` when others are encountered.
    */
    func filter<R: RangeExpression>(statusCodes: R) throws -> Response where R.Bound == Int {
        guard statusCodes.contains(statusCode) else {
            throw NetworkLayerError.statusCode(self)
        }
        return self
    }
    ...
    */
    ...
    func mapImage() throws -> UIImage {
        guard let image = UIImage(data: data) else {
            throw NetworkLayerError.imageMapping(self)
        }
        return image
    }
    ...
    func mapJSON(failsOnEmptyData: Bool = true) throws -> Any {
        do {
            ...
            throw NetworkLayerError.jsonMapping(self)
        }
    }
        ...
        if let keyPath = keyPath {
            ...
            guard let jsonDictionary = try mapJSON() as? NSDictionary,
                let string = jsonDictionary.value(forKeyPath: keyPath) as? String else {
                    throw NetworkLayerError.stringMapping(self)
            }
            return string
        } else {
        ...
        keyPathCheck: if let keyPath = keyPath {
            ...
            if let data = try serializeToData(jsonObject) {
                ...
                do {
                    ...
                    throw NetworkLayerError.objectMapping(error, self)
                }
            }
        } else {
            jsonData = data
